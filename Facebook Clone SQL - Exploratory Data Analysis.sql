...
FROM
    fb_clone.users u
        LEFT JOIN
    fb_clone.photos p ON p.user_id = u.id
WHERE
    p.id IS NULL;
-- --------------------------------------------------------------------------------------------------------------
...
FROM
    fb_clone.photos p
        INNER JOIN
    fb_clone.likes l ON l.photo_id = p.id
        INNER JOIN
    fb_clone.users u ON p.user_id = u.id
GROUP BY p.id
ORDER BY total DESC
LIMIT 1;
...
FROM
    fb_clone.users u
        JOIN
    fb_clone.photos p ON u.id = p.user_id
GROUP BY u.id
ORDER BY 2 DESC
LIMIT 5;
-- --------------------------------------------------------------------------------------------------------------
...
FROM
    ...
    FROM
        fb_clone.users u
    JOIN fb_clone.photos p ON u.id = p.user_id
...
FROM
    fb_clone.tags t
        JOIN
    fb_clone.photo_tags pt ON t.id = pt.tag_id
GROUP BY t.id
...
FROM
    fb_clone.photo_tags pt
        JOIN
    fb_clone.likes l ON l.photo_id = pt.photo_id
        JOIN
    fb_clone.tags t ON pt.tag_id = t.id
GROUP BY 1
ORDER BY 2 DESC
LIMIT 10;
-- --------------------------------------------------------------------------------------------------------------
...
FROM
    fb_clone.users u
        JOIN
    fb_clone.likes l ON u.id = l.user_id
GROUP BY u.id
...
FROM
    ...
    FROM
        fb_clone.users u
    LEFT JOIN fb_clone.comments c ON u.id = c.user_id
...
FROM
    ...
    FROM
        fb_clone.users u
    JOIN fb_clone.likes l ON u.id = l.user_id
    GROUP BY u.id , u.username
    HAVING total_likes_by_user = (SELECT 
            COUNT(*)
        FROM
            fb_clone.photos p)) AS total_number_users_likes_every_photos) AS tableB;
-- --------------------------------------------------------------------------------------------------------------
...
FROM
    ...
    fb_clone.photos p ON p.user_id = u.id
        INNER JOIN
    fb_clone.likes l ON l.user_id = p.user_id
WHERE
    p.id IS NOT NULL
        OR l.user_id IS NOT NULL;
END //

CALL `fb_clone`.`spEngagedUser`();
-- --------------------------------------------------------------------------------------------------------------
...
FROM (
    SELECT 
        ...
        FROM fb_clone.users u
	JOIN fb_clone.comments c ON u.id = c.user_idusers
    WHERE
        c.comment_text IS NOT NULL
    GROUP BY u.username , c.user_id) as Table1;
END //

CALL `fb_clone`.`spUserComments`();
-- --------------------------------------------------------------------------------------------------------------
...
FROM
    ...
    fb_clone.photos p ON p.user_id = u.id
        INNER JOIN
    fb_clone.comments c ON c.user_id = u.id
        INNER JOIN
    fb_clone.photo_tags pt ON pt.photo_id = p.id
        INNER JOIN
    fb_clone.tags t ON t.id = pt.tag_id
where u.id = userid;
END //

CALL `fb_clone`.`spUserInfo`(2);
-- --------------------------------------------------------------------------------------------------------------
